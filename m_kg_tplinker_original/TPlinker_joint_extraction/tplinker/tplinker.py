...
# from m_kg_tplinker_original.TPlinker_joint_extraction.common.components import HandshakingKernel  # Windows
# from common.components import HandshakingKernel  # linux
...
class HandshakingTaggingScheme(object):
    ...
    def __init__(self, rel2id, max_seq_len):
        ...
        self.tag2id_ent = {  # ====
            "O": 0,
            "ENT-H2T": 1,  # entity head to entity tail   # 实体内部的头尾相连矩阵
        }
        self.id2tag_ent = {id_: tag for tag, id_ in self.tag2id_ent.items()}  # ====

        self.tag2id_head_rel = {  # todo 代表了实体之间的关系信息的矩阵 - 实体与实体之间的头部相连矩阵
            "O": 0,
        ...
        self.tag2id_tail_rel = {  # todo 代表了实体之间的关系信息的矩阵 - 实体与实体之间的头尾相连矩阵
            "O": 0,
        ...
        self.shaking_ind2matrix_ind = [(ind, end_ind) for ind in range(self.matrix_size) for end_ind in list(range(self.matrix_size))[ind:]]  # 上三角铺平序列矩阵
        # 没有优化前的完整矩阵，是一个二维矩阵，其上位三角每个元素储存着上三角铺平序列的相对应的位置序号
    ...
    def get_spots(self, sample):
        ...
        for rel in sample["relation_list"]:
            subj_tok_span = rel["subj_tok_span"]  # =====todo save entity
            obj_tok_span = rel["obj_tok_span"]
            ent_matrix_spots.append((subj_tok_span[0], subj_tok_span[1] - 1, self.tag2id_ent["ENT-H2T"]))
            ent_matrix_spots.append((obj_tok_span[0], obj_tok_span[1] - 1, self.tag2id_ent["ENT-H2T"]))  # =====todo save entity
            # todo save 实体头部存储，形式是：【关系类别，实体_1 头部，实体_2头部，关系标签（1,2）】
            ...
            # todo save 实体尾部存储，形式是：【关系类别，实体_1 尾部，实体_2尾部，关系标签（1,2）】
...
class DataMaker4Bert():
    ...
    def get_indexed_data(self, data, max_seq_len, data_type="train"):
        ...
        for ind, sample in tqdm(enumerate(data), desc="Generate indexed train or valid data"):
            ...
            codes = self.tokenizer.encode_plus(text,  # todo max len seq token idx encoder and padding
            ...
            spots_tuple = None
            if data_type != "test":
                spots_tuple = self.handshaking_tagger.get_spots(sample)  # important 开始将 train data 存储在目标结构之中
        ...
        return indexed_samples  # todo 完成Train data编码并返回编码结果
...
class TPLinkerBert(nn.Module):
    def __init__(self, encoder,  # base bert
                 rel_size,
                 shaking_type,
                 inner_enc_type,
                 dist_emb_size,
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        shaking_hiddens4ent = shaking_hiddens  # bert共享编码结果
        shaking_hiddens4rel = shaking_hiddens  # bert共享编码结果
        ...
        ent_shaking_outputs = self.ent_fc(shaking_hiddens4ent)  # todo 基于共享编码抽取实体

        head_rel_shaking_outputs_list = []
        for fc in self.head_rel_fc_list:
            head_rel_shaking_outputs_list.append(fc(shaking_hiddens4rel))  # todo 基于共享编码结果抽取关系实体的尾部信息
