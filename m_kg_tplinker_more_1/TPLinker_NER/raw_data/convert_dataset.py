...
from m_kg_tplinker_more_1.TPLinker_NER.common.utils import Preprocessor
...
def main(data_path, data_type, out_dir, split_train_valid=False, split_radio=0.8, shuffle=True):
    ...
    if "test" in data_type:
        ...
        if not split_train_valid:
            ...
            json.dump(train_dataset, open(os.path.join(out_dir, "mid_train_data.json"), "w", encoding="utf-8"), ensure_ascii=False)
            json.dump(valid_dataset, open(os.path.join(out_dir, "mid_valid_data.json"), "w", encoding="utf-8"), ensure_ascii=False)
